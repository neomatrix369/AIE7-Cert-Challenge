...
def load_and_process_data(csv_path="ragas-evaluation-metrics.csv"):
    ...
    sorted_retrievers = sorted(processed.items(), key=lambda x: x[1]['overall_score'], reverse=True)
...
def create_ragas_heatmap(sorted_data, metrics):
    ...
    rect = patches.Rectangle((-0.5, best_retriever_idx-0.5), len(metrics), 1, 
                           linewidth=5, edgecolor='gold', facecolor='none', alpha=0.8)
    ax.add_patch(rect)
    ...
    for i, (retriever, data) in enumerate(sorted_data):
        rank_color = 'gold' if i == 0 else 'silver' if i == 1 else 'orange' if i == 2 else 'gray'
        ax.text(len(metrics) + 0.3, i, f'#{i+1}', ha='center', va='center', 
               fontsize=14, fontweight='bold', color=rank_color)
    
    # Title and labels
    ax.set_title(f'🎯 RAGAS Metrics Heatmap Analysis\n👑 Best Performer: {best_retriever_name}', 
    ...
    ax.set_yticks(np.arange(len(retrievers)+1)-.5, minor=True)
...
"""
    ...
    fig.text(0.02, 0.98, summary_text, fontsize=9, verticalalignment='top',
...
def create_metric_importance_chart(sorted_data, metrics):
    ...
    ax1.set_title('📊 Metric Importance Weights\n(Used in Overall Scoring)', fontweight='bold')
    ...
    champions = [metric_champions[metric][0].replace('_', ' ').title() for metric in metrics]
...
def main():
    ...
    try:
        ...
        fig1.savefig("ragas_metrics_heatmap.png", dpi=300, bbox_inches='tight', facecolor='white')
        ...
        fig2.savefig("metric_importance_analysis.png", dpi=300, bbox_inches='tight', facecolor='white')
