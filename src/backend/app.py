...
def setup_logger(
    name: str = "ai_makerspace", log_level: str = "INFO"
) -> logging.Logger:
...
logger.info(
    f"   - USE_BROWSER_STORAGE: {os.getenv('USE_BROWSER_STORAGE')} -> {USE_BROWSER_STORAGE}"
)
...
class FileUploadResponse(BaseModel):
    ...
    file_content: Optional[str] = (
        None  # Base64 encoded file content for browser storage
    )
...
def load_vector_database_metadata():
    ...
    try:
        ...
        if is_vercel_environment():
            logger.info(
                "   - Vercel environment detected, skipping file-based metadata loading"
            )
        ...
        if not IS_READONLY and INDEXES_DIR.exists():
            ...
            for index_file in index_files:
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to load metadata from {index_file}: {str(e)}"
                    )
        else:
            logger.info(
                f"   - Skipping metadata load: IS_READONLY={IS_READONLY}, INDEXES_DIR.exists()={INDEXES_DIR.exists()}"
            )
    except Exception as e:
...
def save_file_metadata():
    ...
    if is_vercel_environment():
        logger.info(
            "🔄 Vercel environment detected, skipping file metadata persistence to disk"
        )
        return
...
def load_file_metadata_from_qdrant():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"📊 Found {len(collections.collections)} collections in Qdrant"
            )
        except Exception as e:
            ...
            if collection_name.startswith("documents_"):
                ...
                try:
                    ...
                    try:
                        ...
                        if points[0] and len(points[0]) > 0:
                            ...
                            if metadata and "filename" in metadata:
                                ...
                                logger.info(
                                    f"📊 Found filename in collection scroll: {filename}"
                                )
                    except Exception as e:
                        logger.warning(
                            f"⚠️ Failed to get metadata from scroll: {str(e)}"
                        )
                        ...
                        try:
                            ...
                            if search_results and len(search_results) > 0:
                                ...
                                if metadata and "filename" in metadata:
                                    filename = metadata["filename"]
                                    logger.info(
                                        f"📊 Found filename in search: {filename}"
                                    )
                        except Exception as e:
                            logger.warning(
                                f"⚠️ Failed to get metadata from search: {str(e)}"
                            )
                    ...
                    if filename:
                        ...
                        logger.warning(
                            f"⚠️ No filename found for {file_id}, using default: File_{file_id[:8]}"
                        )
...
if not is_vercel_environment():
    try:
        if FILE_METADATA_PATH.exists():
            ...
            logger.info(
                f"📊 Loaded file_metadata from disk: {len(file_metadata)} files"
            )
        else:
            file_metadata = {}
    ...
    try:
        ...
        for i, row in enumerate(rows):
            row_text = f"Row {i+1}: " + " | ".join(
                [f"{header}: {value}" for header, value in row.items()]
            )
...
async def list_files():
    ...
    if IS_READONLY:
        ...
        for file_id in set(
            list(memory_stored_files.keys()) + list(vector_databases.keys())
        ):
            status_info = indexing_status.get(
                file_id, {"status": "unknown", "message": "File not found"}
            )
            ...
            if file_id in file_metadata:
                ...
                logger.info(
                    f"✅ Found metadata for {file_id}: {actual_filename} ({actual_vector_store_type})"
                )
            elif (
                file_id in vector_databases and "filename" in vector_databases[file_id]
            ):
                actual_filename = vector_databases[file_id]["filename"]
                actual_vector_store_type = (
                    "memory" if file_id in vector_databases else "browser"
                )
                logger.info(
                    f"✅ Found vector database info for {file_id}: {actual_filename}"
                )
            else:
                ...
                if is_vercel_environment() and USE_QDRANT:
                    ...
                    if refreshed_filename:
                        ...
                        logger.info(
                            f"🔄 Refreshed metadata for {file_id}: {actual_filename}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ No metadata found for {file_id}, using generic: {actual_filename}"
                        )
                else:
                    logger.warning(
                        f"⚠️ No metadata found for {file_id}, using generic: {actual_filename}"
            ...
            ready_for_chat = bool(
                status_info["status"] in ["completed", "ready"]
                or file_id in vector_databases
            )
                    ...
                    files.append(
                        {
                            ...
                            "message": status_info["message"],
                            "vector_store_type": (
                                "memory" if file_id in vector_databases else "disk"
                            ),
                            "ready_for_chat": ready_for_chat,
                        }
                    )
...
async def get_file_status(file_id: str):
    ...
    ready_for_chat = bool(
        status_info["status"] in ["completed", "ready"] or in_vector_databases
    )
    ...
    return {**status_info, "ready_for_chat": ready_for_chat}
    ...
    try:
        ...
        if file_type == "pdf":
            ...
            if total_text_length < 10000:  # Small files (< 10KB)
                ...
                logger.info(
                    f"📝 Small file detected ({total_text_length} chars), limiting to {max_chunks} chunks"
                )
            elif total_text_length < 50000:  # Medium files (< 50KB)
                max_chunks = 35
                logger.info(
                    f"📄 Medium file detected ({total_text_length} chars), limiting to {max_chunks} chunks"
                )

            if len(chunks) > max_chunks:
                logger.warning(
                    f"⚠️ Limiting chunks from {len(chunks)} to {max_chunks} for performance"
                )
        ...
        try:
            await vector_db.abuild_from_list(
                chunks, metadata={"file_id": file_id, "filename": filename}
            )
            logger.info(
                f"✅ Successfully indexed {len(chunks)} chunks into vector database"
            )
...
async def chat_with_file(request: FileChatRequest):
    try:
        ...
        if not request.file_ids:
            raise HTTPException(
                status_code=400, detail="At least one file ID is required"
            )
                ...
                if is_vercel_environment() and USE_QDRANT:
                    ...
                    try:
                        ...
                        try:
                            ...
                            search_results = temp_vector_db.search_by_text(
                                "", k=1, return_as_text=False
                            )
                            if search_results and len(search_results) > 0:
                                # Get metadata from the first result
                                metadata = (
                                    search_results[0][1]
                                    if len(search_results[0]) > 1
                                    else {}
                                )
                                if (
                                    ...
                                    logger.info(
                                        f"   - Found filename in Qdrant metadata: {actual_filename}"
                                    )
                                else:
                                    logger.info(
                                        f"   - No filename in Qdrant metadata, using default"
                                    )
                            else:
                                logger.info(
                                    f"   - No documents found in Qdrant, using default filename"
                                )
                        except Exception as e:
                            logger.warning(
                                f"   - Could not retrieve filename from Qdrant: {str(e)}"
                            )

                        # If no metadata found, try to get from file_metadata
                        if (
                            actual_filename == f"File_{file_id[:8]}"
                            and file_id in file_metadata
                        ):
                            actual_filename = file_metadata[file_id].get(
                                "filename", actual_filename
                            )
                            logger.info(
                                f"   - Found filename in file_metadata: {actual_filename}"
                            )
                        ...
                        logger.info(
                            f"   - ✅ File found in Qdrant with filename: {actual_filename}"
                        )
                        continue
                    except Exception as e:
        ...
        for file_id in request.file_ids:
            ...
            if file_data["vector_db"] is None and USE_QDRANT:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Successfully loaded vector database from Qdrant for {file_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to load vector database from Qdrant for {file_id}: {str(e)}"
                    )
                    raise HTTPException(
                        status_code=500,
            ...
            relevant_chunks = vector_db.search_by_text(
                request.user_message, k=2, return_as_text=True
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to chat with file: {str(e)}"
        )
...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        logger.info(
            f"💾 Stored metadata for {file_id}: filename={filename}, vector_store_type={vector_store_type}"
        )
        ...
        if (IS_READONLY and USE_BROWSER_STORAGE and not USE_QDRANT) or (
            is_vercel_environment() and not USE_QDRANT
        ):
            ...
            if is_vercel_environment():
                ...
                logger.info(
                    f"🚀 Starting synchronous indexing for Vercel environment: {file_id}"
                )
                try:
                    ...
                    logger.error(
                        f"❌ Synchronous indexing failed for {file_id}: {str(e)}"
                    )
                    indexing_status[file_id] = {
                        "status": "failed",
                        "message": f"Indexing failed: {str(e)}",
                ...
                logger.info(
                    f"🚀 Starting background indexing for local environment: {file_id}"
                )
...
async def accept_pre_indexed_file(request: PreIndexedFileRequest):
    ...
    try:
        logger.info(
            f"🔍 Processing pre-indexed file: {request.file_id} ({request.filename})"
        )
        ...
        client = OpenAI(
            api_key=api_key, http_client=httpx.Client(timeout=30.0)  # 30 second timeout
        )
        ...
        for i in range(0, len(request.chunks), batch_size):
            ...
            try:
                ...
                logger.info(
                    f"✅ Created embeddings batch {i//batch_size + 1}/{(len(request.chunks) + batch_size - 1)//batch_size} ({len(batch_chunks)} chunks)"
                )
            except Exception as e:
                logger.error(
                    f"❌ Error creating embeddings batch {i//batch_size + 1}: {str(e)}"
                )
                # Update status to failed
        ...
        logger.info(
            f"✅ Created {len(embeddings)} embeddings in {embedding_time:.2f} seconds"
        )
        ...
        for i in range(0, len(request.chunks), insert_batch_size):
            ...
            try:
                ...
                logger.info(
                    f"✅ Inserted batch {i//insert_batch_size + 1}/{(len(request.chunks) + insert_batch_size - 1)//insert_batch_size} into vector database"
                )
            except Exception as e:
                logger.error(
                    f"❌ Error inserting batch {i//insert_batch_size + 1}: {str(e)}"
                )
                # Update status to failed
                indexing_status[request.file_id] = {
                    "status": "failed",
        ...
        logger.info(
            f"✅ Vector database operations completed in {vector_time:.2f} seconds"
        )
        ...
        logger.info(
            f"✅ Successfully indexed file {request.file_id} with {len(request.chunks)} chunks in {total_time:.2f} seconds"
        )
        logger.info(f"📊 Performance breakdown:")
        logger.info(
            f"   - Embedding creation: {embedding_time:.2f}s ({embedding_time/total_time*100:.1f}%)"
        )
        logger.info(
            f"   - Vector database: {vector_time:.2f}s ({vector_time/total_time*100:.1f}%)"
        )
        logger.info(
            f"   - Other operations: {total_time-embedding_time-vector_time:.2f}s ({(total_time-embedding_time-vector_time)/total_time*100:.1f}%)"
        )
        ...
        total_time = time.time() - start_time
        logger.error(
            f"❌ Error indexing file {request.file_id} after {total_time:.2f} seconds: {str(e)}"
        )
        # Update status to failed
        indexing_status[request.file_id] = {
            "status": "failed",
        ...
        if file_id in vector_databases:
            ...
            if USE_QDRANT and hasattr(vector_db, "client"):
                try:
                    ...
                    logger.warning(
                        f"⚠️ Warning: Could not delete Qdrant collection: {str(e)}"
                    )
                    ...
                    try:
                        ...
                        if qdrant_url and qdrant_api_key:
                            ...
                            direct_client.delete_collection(
                                collection_name=collection_name
                            )
                            logger.info(
                                f"✅ Deleted Qdrant collection using direct client for file {file_id}"
                            )
                    except Exception as e2:
                        logger.error(
                            f"❌ Failed to delete Qdrant collection with direct client: {str(e2)}"
                        )
            ...
            try:
                ...
                if file_id in vector_databases:
                    ...
                    if USE_QDRANT and hasattr(vector_db, "client"):
                        try:
                            collection_name = f"documents_{file_id}"
                            vector_db.client.delete_collection(
                                collection_name=collection_name
                            )
                            logger.info(
                                f"✅ Deleted Qdrant collection for file {file_id}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"⚠️ Warning: Could not delete Qdrant collection: {str(e)}"
...
async def test_pdf_processing(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {
            ...
            "total_text_length": sum(len(doc) for doc in documents),
            "sample_text": (
                documents[0][:200] + "..." if documents else "No text extracted"
            ),
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"PDF processing test failed: {str(e)}"
        )
...
def clear_all_qdrant_collections():
    ...
    try:
        ...
        if not qdrant_url or not qdrant_api_key:
            logger.warning(
                "⚠️ Qdrant credentials not found, skipping collection clearing"
            )
            return
