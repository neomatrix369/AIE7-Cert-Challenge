...
async def ask_student_loan_question(request: StudentLoanQuestion):
    ...
    try:
        ...
        try:
            ...
            logger.info("✅ RAG agent response received")
        except Exception as e:
        ...
        if not response or "messages" not in response:
            raise HTTPException(
                status_code=500,
                detail="Invalid response from RAG agent"
            )
        ...
        if not messages:
            raise HTTPException(
                status_code=500,
                detail="No response generated by RAG agent"
            )
        ...
        sources_count = 5  # Parent Document retriever typically returns 5 contexts
        
        logger.info(f"✅ Question processed successfully, response length: {len(answer)}")
...
if __name__ == "__main__":
    ...
    required_vars = ["OPENAI_API_KEY", "TAVILY_API_KEY", "LANGCHAIN_API_KEY", "LANGCHAIN_TRACING_V2", "LANGCHAIN_PROJECT"]
